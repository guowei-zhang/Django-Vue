	...
	<div class="adminui-tags">
		<ul>
			<li v-for="tag in tagList" v-bind:key="tag" :class="isActive(tag) ? 'active' : '' " @contextmenu.prevent="openContextMenu($event, tag)">
				...
				<icon :class="tag.meta.icon || 'el-icon-menu'"></icon> {{ tag.meta.tagTitle }}
